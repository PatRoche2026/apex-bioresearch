...
def ingest_role(role: str, force: bool = False) -> int:
    ...
    for filepath in files:
        ...
        # Embed one chunk at a time with delay to respect rate limits
        for i, chunk in enumerate(chunks):
            if i > 0:
                print(f"    Waiting {EMBED_DELAY}s for rate limit...")
                time.sleep(EMBED_DELAY)
            add_documents(role, [chunk], metadatas=[metadatas[i]], ids=[ids[i]])
